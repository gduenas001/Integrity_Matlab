    ...
    if dtsum >= DT_OBSERVE
        ...
        if ~isempty(z)
            ...
            elseif SWITCH_ASSOCIATION == 2
                [gamma,H,Y,idf,PCA(step),PCA_MJ(step),PCAt(step),allOutliers]= ...
...
%                 PX= (eye(3) - K*H)*PX;
    ...
    calcPCA_MJ(step)= sum(PCA_MJ)/step;
...
figure; hold on; grid on
plot(realPCA,'-r','linewidth',2);
plot(calcPCA,'b-','linewidth',2);
plot(calcPCA_MJ,'g-','linewidth',2);
